...
class MCAttention(nn.Module):
    ...
    def generate_len_scale_mask(self, padding_masks):
        ...
        for padding_mask in padding_masks:
            ...
            seq_len = torch.logical_not(padding_mask).sum(1)
            max_len_ls.append(torch.tensor(max_len))
            seq_len_ls.append(seq_len)
            balance_factor = seq_len**-0.5 #(bsz,)
            scale_mask = balance_factor.view(bsz, 1).expand(-1,max_len)
            scale_mask_ls.append(scale_mask)
        extrapolation_factor = torch.log(torch.sum(torch.stack(seq_len_ls),0).float())/torch.log(torch.sum(torch.stack(max_len_ls),0).float())
        return extrapolation_factor.view(bsz, 1)*torch.cat(scale_mask_ls,dim=1)
